...
public abstract class DefaultScheduleTable extends JPanel {
	...
	private int dayHours[] = new int[7];
	
	private ShiftTab[][] shiftTabTable;
	
	public DefaultScheduleTable(ShiftDefaultList sdList) {
		setSdList(sdList);
		for(int i=0;i<7;i++) {
			for(int j=0;j<6;j++) {
	...
	private void loadShifts() {
		shiftTabTable = new ShiftTab[7][6];
		for(int i = 0; i<7; i++) {
			for(int j = 0; j<6; j++) {
				shiftTabTable[i][j] = new ShiftTab(i,j) {
					@Override
					public void click() {
						toggleShift(getI(),getJ());
					}
				};
		...
		for(int i = 0; i<7; i++) {
			for(int j=0; j<6; j++) {
				if(getSdList().getShift(i, j)) {
					...
					if(j==5) {
						for(int k=0;k<6;k++) {
							if(sdList.getShift(ip,k)) {
								removeShift(ip,k);
							}
							shiftTabTable[ip][k].setAvailable(false);
						}
					}
				} else {
					shiftTabTable[i][j].setPresent(false);
					if(getTotalHours()==Application.maxWeekHours) {
						shiftTabTable[i][j].setAvailable(false);
					}
					if(dayHours[i]==Application.maxDayHours) {
						for(int x = 0; x<6; x++) {
							if(!sdList.getShift(i,x)) {
								shiftTabTable[i][x].setAvailable(false);
							}
						}
					}
				}
	...
	public void toggleShift(int i, int j) {
		if(sdList.getShift(i,j)) {
			removeShift(i,j);
		} else {
			addShift(i,j);
	...
	public abstract void updateTotalHours();
	
	public void addShift(int i, int j) {
		totalHours+=4;
		dayHours[i]+=4;
		getSdList().setShift(i, j);
		refreshShift();
	}
	
	public void removeShift(int i, int j) {
		totalHours-=4;
		dayHours[i]-=4;
		getSdList().delShift(i, j);
		refreshShift();
	}
